...
用法: 
搜索: python file_search_replace_mt.py -d <目录> -t <文件类型> -c <搜索内容> [-o <输出文件>] [-j <线程数>]
替换: python file_search_replace_mt.py -d <目录> -t <文件类型> -c <搜索内容> -r <替换内容> [-o <输出文件>] [-b] [-j <线程数>]

示例: 
搜索: python file_search_replace_mt.py -d /home/user/project -t py -c "import os" -o path.log -j 4
替换: python file_search_replace_mt.py -d /home/user/project -t py -c "import os" -r "import sys" -b -j 4
"""
...
# 线程安全的结果计数器
result_counter = {'found': 0, 'replaced': 0, 'errors': 0}
# 线程锁，用于更新计数器
                    ...
                    try:
                        ...
                        if backup:
                            try:
                                ...
                                log_queue.put(f"创建备份失败 {file_path}: {str(e)}")
                                # 如果备份失败但要求备份，则跳过替换
                                if backup:
                                    raise Exception(f"备份失败，跳过替换: {str(e)}")
                        ...
                        try:
                            ...
                            log_queue.put(f"写入文件失败 {file_path}: {str(e)}")
                    except Exception as e:
                        log_queue.put(f"替换过程中出错 {file_path}: {str(e)}")
...
def logger_thread(log_file_path, stop_event):
    ...
    try:
        ...
        print(f"日志线程出错: {str(e)}")

def search_and_replace_files(directory, file_type, content, output_file="path.log", replace_with=None, backup=False, num_threads=4):
    """
    多线程在指定目录中搜索包含指定内容的指定类型文件，并可选择替换内容
    ...
    Returns:
        tuple: (找到匹配内容的文件数量, 替换操作的文件数量, 错误文件数量)
    """
    ...
    try:
        with open(output_file, 'w', encoding='utf-8') as log_file:
            ...
            header += f"目录: {os.path.abspath(directory)}\n"
        ...
        print(f"无法写入日志文件 {output_file}: {str(e)}")
        return 0, 0, 0
    ...
    print(f"找到 {file_count} 个 .{file_type} 文件，开始处理...")
    ...
    try:
        with open(output_file, 'a', encoding='utf-8') as log_file:
            ...
            summary += f"- 处理过程中出现 {result_counter['errors']} 个错误\n"
            summary += f"- 总耗时: {elapsed_time:.2f} 秒\n"
            
            print(summary)
            log_file.write(summary)
    except Exception as e:
        print(f"写入摘要时出错: {str(e)}")
    
    return result_counter['found'], result_counter['replaced'], result_counter['errors']

def main():
    """主函数，处理命令行参数并执行搜索/替换"""
    parser = argparse.ArgumentParser(description='多线程在指定目录中搜索和替换指定类型文件中的内容')
    parser.add_argument('-d', '--directory', required=True, help='要搜索的目录路径')
    ...
    # 限制线程数在合理范围内
    num_threads = max(1, min(args.jobs, 32))
    
    try:
        # 执行搜索和可选的替换
        found_count, replaced_count, error_count = search_and_replace_files(
            args.directory, 
            args.type, 
            args.content, 
            args.output, 
        ...
        if error_count > 0:
            print(f"注意: 处理过程中出现 {error_count} 个错误，详情请查看日志")
